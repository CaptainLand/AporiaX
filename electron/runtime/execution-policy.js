...
const RULES = Object.freeze([
  ...
  },
  {
    ...
    reason: "The command can irreversibly rewrite or delete remote state.",
    patterns: [
      /\bgit\s+push\b[^\n]*(?:--force-with-lease|--force|-f)\b/i,
      /\bgh\s+pr\s+(?:merge|close)\b/i,
      /\bgh\s+release\s+(?:delete|edit)\b/i,
  ...
  },
  {
    ...
    reason: "Dependency changes are local, reversible through the lockfile, and verifiable by the Harness.",
    patterns: [
  ...
  },
  {
    ...
    reason: "The command performs a read-only network fetch and does not intentionally mutate remote state.",
    patterns: [
      /(?:^|[;&|]\s*)curl\b(?![^\n]*(?:-X\s*(?:POST|PUT|PATCH|DELETE)|--request\s+(?:POST|PUT|PATCH|DELETE)|--data(?:-raw|-binary|-urlencode)?\b|-d\b|--form\b|-F\b|--upload-file\b|-T\b))/i,
  ...
  },
  {
    ...
    reason: "The remote write is scoped to a normal branch or review artifact and is reversible.",
    patterns: [
      /^\s*git\s+push\b(?![^\n]*(?:--force-with-lease|--force|-f)\b)/i,
