...
function sameTarget(previous, tool, input, context) {
...
async function reconcile(context, tool, input, fingerprint, requestApproval) {
  if (REPLAY_SAFE.has(tool)) return;
  const getMatches = async () => {
    const pending = (context.unresolved || []).filter((op) => !REPLAY_SAFE.has(op.tool) && (
      op.fingerprint === fingerprint || sameTarget(op, tool, input, context) ||
      (!op.fingerprint && op.tool === tool) || op.tool === "legacy-run"
    ));
...
export async function executeDurableTool(tool, input, execute, requestApproval) {
  const context = storage.getStore();
  if (!context || READ_ONLY.has(tool)) return execute();
  const operationId = randomUUID();
  const fingerprint = createHash("sha256").update(JSON.stringify([tool, stableInput(input)])).digest("hex");
  await reconcile(context, tool, input, fingerprint, requestApproval);
  abortIfNeeded(context);
  const target = typeof input?.path === "string" ? input.path : undefined;
  const intent = { operationId, tool, target, cwd: input?.cwd || ".", fingerprint, state: "started" };
