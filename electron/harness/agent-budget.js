...
const DELEGATION_PATTERN = /(?:\b(?:delegate|delegation|subagent)\b|\bin\s+the\s+background\b|委派|交给子 ?agent|调用子 ?agent|后台(?:探索|调查|检查|运行|执行))/i;
...
const VERIFICATION_PATTERN = /(?:\b(?:build|lint|test|typecheck|verify|verification|self[- ]?check)\b|构建|测试|验证|检查|自检|复核|校验)/i;
...
function classifyRequest(options = {}) {
  ...
  const explicitDelegation = DELEGATION_PATTERN.test(text);
  ...
  if (!mutating) {
    if (
      investigating &&
      (text.length > 220 || pathMentions > 0 || mentionsAgents || explicitDelegation)
    ) {
