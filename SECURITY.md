...
- 当前源码已移除手机伴侣同步与远程控制：没有用于后台同步本地项目、完整对话、任务快照或浏览/上传电脑文件的账户通道。
- 登录仅处理账户、额度和设备等必要信息；它不触发本地任务/文件同步。旧手机通道的专用缓存会在账户模块启动时按固定文件名清理，不删除本地对话或项目。
- **本地优先不等于完全离线。** 使用 Aporia Cloud 或第三方 API 模型时，推理所需的提示词、相关对话、工具返回的文件片段，以及明确提供的附件内容仍会发送到所选模型服务。Aporia Cloud 会经过其模型网关；这不是手机同步功能。
- 用户授权的 Git 推送、浏览器、联网命令和 MCP 也可能发送数据；不希望任何内容离开电脑时，应使用本地模型并限制这些网络能力。移除手机通道不构成“零数据外发”的承诺。
