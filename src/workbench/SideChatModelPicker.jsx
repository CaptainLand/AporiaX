...
export function SideChatModelPicker({ options, choice, disabled, onSelect }) {
  const { tr } = useI18n();
  ...
  const choose = (option) => { onSelect(option.value); close(true); };
  const navigate = (event) => {
    if (event.key === "Escape") { event.preventDefault(); event.stopPropagation(); close(true); return; }
    if (event.nativeEvent.isComposing) return;
    const rows = [...rootRef.current.querySelectorAll('[role="option"]')];
    ...
    if (event.key === "ArrowDown" || event.key === "ArrowUp") {
      ...
      event.preventDefault(); if (filtered[0]) choose(filtered[0]);
    }
  };
  const ModelIcon = choice?.icon;
  return <div className="side-chat-model-picker" ref={rootRef}>
    <button ref={triggerRef} className={"model-trigger side-chat-model-trigger" + (open ? " active" : "")} type="button" aria-label={tr("侧聊模型", "Side chat model")} aria-haspopup="dialog" aria-expanded={open} aria-controls={open ? popupId : undefined} disabled={disabled || !options.length}
      ...
      {ModelIcon && <ModelIcon size={15} />}<span>{choice ? sideChatModelLabel(choice) : tr("请先配置模型", "Configure a model first")}</span><ChevronDown size={13} />
    </button>
      ...
      <div className="side-chat-model-options" role="listbox" aria-label={tr("可用模型", "Available models")}>
        {groups.map((group) => <div role="group" aria-label={group.name} key={group.id}>
          ...
          {group.options.map((option) => { const Icon = option.icon; const selected = choice?.value === option.value;
            return <button type="button" role="option" aria-selected={selected} key={option.value} className={"side-chat-model-option" + (selected ? " selected" : "")} title={option.providerName + " · " + option.modelId} onClick={() => choose(option)}>
              <Icon size={16} /><span><strong>{sideChatModelLabel(option)}</strong><small>{option.modelId}</small></span>{selected && <Check size={15} />}
            </button>;
          })}
        </div>)}
